...
class ModelArguments:
    ...
    scale_factor: Optional[int] = field(default=2)
...
def preprocess_hermes_43(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    has_image: bool = False
) -> Dict:
    """
    Hermes-4.3 preprocessing with chat-template rendering.
    Supports multimodal LLaVA-style <image> placeholders.

    Key idea:
    - Do NOT use conv.get_prompt()/sep splitting.
    - Render messages with tokenizer.apply_chat_template(..., tokenize=False).
    - Then tokenize the rendered prompt:
        * tokenizer_image_token(...) if has_image
        * tokenizer(...) otherwise
    - Compute assistant label spans by incremental prefix rendering.
    """
    ...
    def normalize_content(value):
        """
        Preserve <image> markers for multimodal examples.
        Handle tuple payloads used by LLaVA datasets.
        """
        if isinstance(value, tuple):
            # Common LLaVA tuple forms:
            #   (text, image, image_process_mode)
            #   (text, images)
            text = value[0]
            if len(value) >= 2:
                maybe_images = value[1]
                if isinstance(maybe_images, (list, tuple)):
                    num_images = len(maybe_images)
                    if num_images > 0 and "<image>" not in text:
                        text = "<image>" * num_images + text
                else:
    ...
    def tokenize_rendered(rendered_text: str) -> torch.Tensor:
        if has_image:
    ...
    for idx, source in enumerate(sources):
        if len(source) == 0:
            raise ValueError(f"Empty conversation at index {idx}")

        # Match legacy behavior loosely
        if source[0]["from"] not in {"human", "system"}:
            source = source[1:]
        ...
        for turn in source:
            messages.append(
                {
                    "role": map_role(turn["from"]),
                    "content": normalize_content(turn["value"]),
                }
            )

        # Render full conversation with tokenizer chat template
        ...
        full_ids = tokenize_rendered(rendered_full)
        labels = full_ids.clone()
        labels[:] = IGNORE_INDEX

        # Unmask only assistant spans using incremental prefix rendering
        prev_len = 0
        prefix_messages = []
        ...
        for msg in messages:
            ...
            cur_ids = tokenize_rendered(rendered_prefix)
            cur_len = cur_ids.shape[0]

            if msg["role"] == "assistant":
                labels[prev_len:cur_len] = full_ids[prev_len:cur_len]
...
def preprocess_qwen_2(
    ...
    messages = [
        ...
        {"role": "user", "content": sources[0][0]['value']},
        {"role": "assistant", "content": sources[0][1]['value']},
    ]
    ...
    if has_image:
        input_ids=tokenizer(conversations, return_tensors='pt').input_ids
        #input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    else:
        input_ids = tokenizer(
            conversations,
    ...
    part=conversations[0].split(sep)
    part[0]+=sep
  
    prompt_len=len(tokenizer(part[0]).input_ids)
    targets[0][:prompt_len] = IGNORE_INDEX
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "chatml_direct":
        return preprocess_mpt(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.Qwen3:
        return preprocess_qwen_2(sources,tokenizer,has_image=has_image)
    if conversation_lib.default_conversation.version == "llama3":
        return preprocess_llama3(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "hermes_43":
        return preprocess_hermes_43(sources, tokenizer, has_image=has_image)    
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        elif "Hermes-4.3" in model_args.model_name_or_path:
        ...
        elif "Nous-Hermes" in model_args.model_name_or_path:
    ...
    if model_args.version == "v0":
    ...
    elif "Hermes-4.3" in model_args.model_name_or_path or "Hermes-4.3-36B" in model_args.model_name_or_path:
        if tokenizer.pad_token is None:
        ...
        conversation_lib.default_conversation = conversation_lib.conv_templates["llama3"]  # placeholder
        conversation_lib.default_conversation.version = "hermes_43"
        print("Using conversation format: hermes_43")
    else:
        if tokenizer.pad_token is None:
